...
const P_DEFAULT_PREFIXES = '/context/default';
const P_DEFAULT_PLUGIN_PACK = '/pack/default';
		...
		// it is a prefixed name
			...
			if(m_prefixed_name) {
				...
				if(!h_prefixes[s_prefix]) {
					throw `no such prefix found in context: ${s_prefix}`;
				}
			...
			// none of the above
				...
				throw 'failed to decode resource from hash';
			}
		}
	}
...
document.addEventListener('DOMContentLoaded', () => {
	...
	async.waterfall([
		...
		(fk_task) => {
			// browsing named source
			let m_source = R_BROWSE_SOURCE.exec(s_path);
			if(m_source) {
				// get source
				let s_endpoint = m_source[1];

				// fetch endpoint config
				fetch_endpoint_config(s_endpoint, (e_download, h_source) => {
					// set endpoint url
					p_endpoint = s_endpoint;

					// endpoint url missing protocol
					if(!/^\w+:\/\//.test(p_endpoint)) {
						p_endpoint = 'http://'+p_endpoint;
					}
		...
		(fk_task) => {
			...
			if('string' === typeof z_prefixes) {
				...
				if(z_prefixes.startsWith('/')) {
					z_prefixes = location.origin+z_prefixes;
				}
		...
		(h_loaded_prefixes, fk_task) => {
			...
			// extract resource
			let p_resource = extract_resource(window.location.hash.substr(1), true);
		...
		window.addEventListener('hashchange', () => {
			...
			let p_resource = extract_resource(window.location.hash.substr(1));
